...
/* Funcao que insere uma nova aresta no grafo ou atualiza o valor de uma aresta
   existente. Se os vertices passados como parametro forem validos, ha duas
   possibilidades: a aresta nao existia ou a aresta existia (atualizacao de
   peso).
*/
...
/* Funcao que retorna o peso da aresta, se existe uma aresta valida entre v1 e
   v2 e -1 caso contrario.
*/
...
void caixeiroAux(Grafo *g, int atual, int numVisitados) {
  ...
  visitado[atual] = true;
  cicloAtual[numVisitados - 1] = atual;

  //condição de parada: todos os vértices visitados e o próximo vizinho do último vértice ser o vértice 0
  if (numVisitados == g->numVertices && arestaExiste(g, atual, 0)) {
    valorAtual = valorAtual + pesoAresta(g, atual, 0);
    //se a solução atual for a melhor solução por enquanto, salve-a.
    ...
    //desfaça o final do ciclo atual
    valorAtual = valorAtual - pesoAresta(g, atual, 0);
    visitado[atual] = false;
    return;
  }

  //teste todos os vizinhos do vértice atual
  ElemLista *end = g->A[atual];
  while (end) {
    //se o próximo vizinho do vértice atual não foi visitado, visite e chame recursivamente a função a partir dele.
    if (!visitado[end->vertice]) {
      valorAtual = valorAtual + end->peso;
      caixeiroAux(g, end->vertice, numVisitados + 1);
      //Desfaça o valor do ciclo atual para tentar outras opções. A melhor opção até então já está salva.
      valorAtual = valorAtual - end->peso;
    }
    end = end->prox;
  }

  //desmarca o vértice atual para tentar outro caminho e poder visitá-lo em caminhos futuros
  visitado[atual] = false;
}
